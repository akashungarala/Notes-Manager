...
class Login: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        if (FIRAuth.auth()?.currentUser != nil) {
            performSegueWithIdentifier("NotebooksSegueFromLogin", sender: nil)
        }
    }
        ...
        if email.text == "" {
            ...
            FIRAuth.auth()!.signInWithEmail(self.email.text!, password: self.password.text!, completion: { (user, error) in
                if error != nil {
                    ...
                    self.performSegueWithIdentifier("NotebooksSegueFromLogin", sender: sender)
                    self.activityIndicatorView.stopAnimating()
                }
            })
        }
    ...
    override func shouldPerformSegueWithIdentifier(identifier: String, sender: AnyObject?) -> Bool {
        if identifier == "NotebooksSegueFromLogin" {
            return false
        }
        return true
    }
